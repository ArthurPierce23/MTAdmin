...
from PySide6.QtWidgets import (
    ...
    QTabWidget, QMenu, QMainWindow, QMessageBox, QInputDialog, QSpacerItem, QSizePolicy
)
...
class DynamicTabs(QTabWidget):
    def __init__(self, with_initial_tab=True, theme_name="Светлая"):
        ...
        self.pc_connection_block.connection_successful.connect(self.handle_connection)
        ...
        self.addTabButton.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
        self.addTabButton.setMinimumSize(40, 40)  # Минимальный размер
        self.addTabButton.setMaximumSize(40, 40)  # Максимальный размер
    ...
    @staticmethod
    def create_tab_content(parent_tabs: 'DynamicTabs', is_pc_connection_needed: bool = True) -> QWidget:
        """
        ...
        # Создаем блок для отображения недавних подключений
        recent_connections_block = RecentConnectionsBlock()

        if is_pc_connection_needed:
            # Создаем блок подключения и подключаем его сигнал к методу родительской вкладки
    ...
    def handle_connection(self, os_name: str, pc_name: str, ip: str):
        """Обрабатывает успешное подключение и заменяет содержимое вкладки новым окном."""
        # Определяем, из какого блока был исходит сигнал
        sender = self.sender()
        tab_index = None
        for i in range(self.count()):
            # Если виджет текущей вкладки является предком (ancestor) отправителя сигнала,
            # значит, этот таб отвечает за подключение.
            if self.widget(i).isAncestorOf(sender):
                tab_index = i
                break
        ...
        # В зависимости от ОС создаем нужное окно и формируем заголовок
        if os_name == "Windows":
            new_widget = WindowsWindow(pc_name, ip)
            new_title = f"Windows: {pc_name}"
        elif os_name == "Linux":
            new_widget = LinuxWindow(ip)
            new_title = f"Linux: {ip}"
        else:
            QMessageBox.warning(self, "Ошибка", "Не удалось определить операционную систему.")
            return

        # Заменяем содержимое вкладки:
        # Удаляем старую вкладку и вставляем новую с тем же индексом
        self.removeTab(tab_index)
        self.insertTab(tab_index, new_widget, new_title)
    ...
    def open_windows_gui(self, hostname: str, ip: str):
        """Метод для открытия вкладки с Windows-окном."""
        try:
            windows_widget = WindowsWindow(hostname, ip)
            self.add_existing_tab(windows_widget, f"Windows: {hostname}")
            self.setCurrentIndex(self.count() - 1)  # Переключаемся на только что открытую вкладку
    ...
    def open_linux_gui(self, ip: str):
        """Метод для открытия вкладки с Linux-окном."""
        try:
